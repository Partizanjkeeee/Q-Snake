...
void SRecord::as_string(char* buffer) {
	sprintf(buffer, recordFormatStr, name, rating, lenght, game_time, ctime(&date));
}
...
CGame::CGame(CScreen& _scr, int _widght, int _height, int _latency) :
	width(_widght), height(_height), latency(_latency), scr(_scr) {
...
void CGame::top10(bool after_game) {
	scr.cls();
	char buf[80];
	TCHAR buf_encoded[NAMELENGHT];

	top10_table();									//показывать таблицу 10 лучших результатов

	time_t date = time(NULL);
	if (after_game) {
		//если игра была сыграна, то показать следующий результат
		scr.pos(5, 16);
		_cprintf(recordFormatStr, "Your result", rating, snake.size(), duration_game, ctime(&date));
	}

	if (rating > ttop10[9].rating) {				//если рейтинг игры больше, чем меньший из 10лучших...
		//запросить имя игрока
		scr.pos_str(5, 20, "Your name: _");
		scr.pos(16, 20);
		cin.getline(&buf[0], NAMELENGHT);
		clearkeys();
		OemToCharBuff(buf, buf_encoded, static_cast<DWORD>(NAMELENGHT));
		//заменить последнюю запись 10 лучших результатов
		ttop10[9].name, buf_encoded;
		ttop10[9].date = date;
		ttop10[9].game_time = duration_game;
		ttop10[9].lenght = snake.size();
...
const char* copyright = "(c) Djiinn, 2023.";

void CGame::logo() {
	scr.pos_str(width / 2 - 9, 10, "Q");
...
void CGame::game_loop() {
	...
	snake.reset(SCoord(width / 2, height / 2));   // установить зиею: длинна 2,
												  // положение - в середине игрового поля, направление - влево
	...
	do {
		...
		if (_kbhit())				// если в буфере клавиатуры есть информация, то принять команду
			cmd = get_command();
		...
		// если голова змеи столкнулась со стеной или с телом змеи, то змея DIE!!!
		if (hd.x == 0 || hd.x == width - 1 || hd.y == 0 || hd.y == height - 1 || snake.into(hd))
			stt = STATE_DIED;

		if (stt == STATE_OK) {			// если змея еще жива, то
