...
const int NAMELENGHT = 16;					// размер буфера для имени игрока						\\ГДЕ_ТО ТУТ У МЕНЯ ОШИБКА!!! ТУТ И В СТРУКТУРЕ SRecord!!!!

// Структура для хранения результата игры

struct SRecord {
	char name[NAMELENGHT];				// имя игрока
	double rating;						// рейтинг
	int lenght;							// длина змеи
