...
const char SNAKE_TALE = '@';				// символ для отрисовки сегментов тела змеи, кроме головы
...
void CSnake::draw(CScreen& scr) {
	...
	for (unsigned int i = 0; i < wsize; i++)
		scr.pos(worm[i].x, worm[i].y, SNAKE_TALE);
...
void CSnake::move(const SCoord& delta, CScreen& scr) {
	...
	scr.pos(worm[worm.size() - 2].x, worm[worm.size() - 2].y, SNAKE_TALE);
}
